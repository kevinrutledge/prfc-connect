...
- **Software Supply Chain Failures (A03)** are limited by a committed lock file with integrity hashes, a clean `npm audit`, and pinned `package.json` overrides for transitive fixes. The runtime and frameworks are current, running Node 22, Next.js 16, Prisma 7, and React 19.
